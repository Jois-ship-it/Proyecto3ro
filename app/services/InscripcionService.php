...
class InscripcionService
{
    ...
    public function inscribirParticipante(int $torneoId, int $participanteId): void
    {
        ...
        // Si existe una fila previa (p. ej. retirada), reactivar en lugar de insertar
        $existing = $this->model->findInscripcion($torneoId, $participanteId, null);
        if ($existing) {
            $this->model->reactivar((int)$existing['id']);
            $this->auditoria->log('inscribir_participante', 'inscripciones', (int)$existing['id'], "Participante {$participanteId} inscrito en torneo {$torneoId}");
        } else {
            ...
            $this->auditoria->log('inscribir_participante', 'inscripciones', $newId, "Participante {$participanteId} inscrito en torneo {$torneoId}");
        }
    }
    ...
    public function inscribirEquipo(int $torneoId, int $equipoId): void
    {
        ...
        // Si existe una fila previa (p. ej. retirada), reactivar en lugar de insertar
        $existing = $this->model->findInscripcion($torneoId, null, $equipoId);
        if ($existing) {
            $this->model->reactivar((int)$existing['id']);
            $this->auditoria->log('inscribir_equipo', 'inscripciones', (int)$existing['id'], "Equipo {$equipoId} inscrito en torneo {$torneoId}");
        } else {
            $newId = $this->model->insert([
                'torneo_id' => $torneoId,
                'equipo_id' => $equipoId,
            ...
            $this->auditoria->log('inscribir_equipo', 'inscripciones', $newId, "Equipo {$equipoId} inscrito en torneo {$torneoId}");
        }
    }
