...
# (AlmaLinux 9.3 + Apache/httpd + PHP + MariaDB).
#
...
# Opciones:
#   --no-deps        No instalar paquetes del sistema (dnf)
#   --no-vhost       No configurar el VirtualHost de Apache
#   --root-pass PWD  Contraseña de root de MySQL/MariaDB (si no usa auth por socket)
#
...
# ---- Usuario/grupo del servidor web en RHEL/AlmaLinux ----
WEB_USER="apache"
WEB_GROUP="apache"
...
echo "==> SO:         AlmaLinux 9.3 (dnf / httpd / mariadb)"
echo

# ---- 1) Dependencias del sistema ----
if [ "$INSTALL_DEPS" -eq 1 ]; then
  echo "==> Instalando paquetes (httpd, php, mariadb-server)..."
  dnf install -y epel-release >/dev/null 2>&1 || true
  dnf install -y httpd mariadb-server mariadb \
      php php-cli php-mysqlnd php-gd php-mbstring php-xml php-json \
...
SQL
  ...
  echo "    Además, habilitá el booleano SELinux para conexiones salientes:"
  echo "    setsebool -P httpd_can_network_connect_db 1"
  setsebool -P httpd_can_network_connect_db 1 2>/dev/null || true
fi

# ---- 3) Permisos ----
echo "==> Ajustando permisos ($WEB_USER)..."
chown -R "$WEB_USER:$WEB_GROUP" "$PROJECT_ROOT"
find "$PROJECT_ROOT" -type d -exec chmod 755 {} \;
find "$PROJECT_ROOT" -type f -exec chmod 644 {} \;

# ---- 3.1) Contexto SELinux ----
if command -v semanage >/dev/null 2>&1 || command -v chcon >/dev/null 2>&1; then
  echo "==> Aplicando contexto SELinux (httpd_sys_content_t)..."
  semanage fcontext -a -t httpd_sys_content_t "${PROJECT_ROOT}(/.*)?" 2>/dev/null || true
  restorecon -Rv "$PROJECT_ROOT" >/dev/null

  # Si el proyecto necesita escribir en algún directorio (uploads, logs, cache),
  # descomentar y ajustar la ruta correspondiente:
  # semanage fcontext -a -t httpd_sys_rw_content_t "${PROJECT_ROOT}/storage(/.*)?" 2>/dev/null || true
  # restorecon -Rv "$PROJECT_ROOT/storage" >/dev/null
else
  echo "==> AVISO: no se encontró semanage/chcon; se omite el ajuste de contexto SELinux."
fi

# ---- 4) VirtualHost de Apache (httpd) ----
if [ "$CONFIG_VHOST" -eq 1 ]; then
  CONF="/etc/httpd/conf.d/${PROJECT}.conf"
  echo "==> Escribiendo $CONF ..."

  # En httpd no existe ports.conf: los "Listen" van en httpd.conf.
  if [ "$HTTP_PORT" != "80" ] && [ "$HTTP_PORT" != "443" ]; then
    if ! grep -qE "^[[:space:]]*Listen[[:space:]]+$HTTP_PORT([[:space:]]|$)" /etc/httpd/conf/httpd.conf; then
...
<VirtualHost *:$HTTP_PORT>
    ...
    ErrorLog  /var/log/httpd/${PROJECT}_error.log
    CustomLog /var/log/httpd/${PROJECT}_access.log combined
</VirtualHost>
VHOST
  ...
  if [ "$HTTP_PORT" = "80" ] || [ "$HTTP_PORT" = "443" ]; then
    ...
    [ "$HTTP_PORT" = "443" ] && firewall-cmd --permanent --add-service=https >/dev/null 2>&1 || true
  else
